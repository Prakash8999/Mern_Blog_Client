		...
		{post.length>0 &&
         ...
         return(
           <div key={index} className="p-5 flex items-center justify-center">
            ...
            <PostCard title={value?.title} summary={value?.summary} content={value?.content} createdAt={format(new Date (value?.createdAt), 'd MMM, yyyy h:mm')} image={value?.image} id={value?._id} author={value?.author} />
...
<button className=" text-6xl hover:text-yellow-500 duration-200 shadow drop-shadow-lg  fixed bottom-2 bg-black text-white bg-opacity-60 rounded-full md:hidden block" onClick={plus} ><IoAddOutline/></button>
</div>
    </div>
    </>
  );
